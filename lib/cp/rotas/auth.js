...
async function registrar({ body, res }) {
  ...
  try {
    ...
    return ok(res, { ok: true, token, usuario: { id, nome, email, pergunta_recuperacao: pergunta } }, 201);
  } catch (e) {
...
async function login({ body, res }) {
  ...
  try {
    ...
    const { id, password_hash, nome, pergunta_recuperacao } = r.rows[0];
    ...
    return ok(res, { ok: true, token, usuario: { id, nome: nome || null, email, pergunta_recuperacao: pergunta_recuperacao || null } });
  } catch (e) {
...
async function me({ usuarioId, res }) {
  try {
    const r = await db.withUser(usuarioId, (c) =>
      c.query('SELECT id, email, nome, telefone, pergunta_recuperacao, criado_em FROM usuarios WHERE id = $1', [usuarioId]));
