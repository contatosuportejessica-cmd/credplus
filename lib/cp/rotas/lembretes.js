...
async function vinculoCobranca(c, lembrete, usuarioId) {
  ...
  if (lembrete.parcela_id == null) {
    const r = await c.query(
      `SELECT e.id AS emprestimo_id, cl.id AS cliente_id, cl.nome AS cliente_nome, cl.telefone, cl.whatsapp
       ...
       WHERE e.id = $1 AND e.usuario_id = $2 AND cl.usuario_id = $2`, [lembrete.emprestimo_id, usuarioId]);
  ...
  const r = await c.query(
    ...
    [lembrete.parcela_id, lembrete.emprestimo_id, usuarioId]);
  if (!r.rows.length) return null;
  const v = r.rows[0];
  return {
...
async function comCobranca(c, lembrete, clienteNome) {
  const cobranca = await vinculoCobranca(c, lembrete, usuarioId);
...
async function validarVinculo(c, { clienteId, emprestimoId, parcelaId }, usuarioId) {
  if (emprestimoId == null && parcelaId == null) return null;
  if (emprestimoId != null) {
    const r = await c.query('SELECT id, cliente_id FROM emprestimos WHERE id = $1 AND usuario_id = $2', [emprestimoId, usuarioId]);
  ...
  if (parcelaId != null) {
    const r = await c.query('SELECT id, emprestimo_id FROM parcelas WHERE id = $1 AND usuario_id = $2', [parcelaId, usuarioId]);
    ...
    const resultado = await db.withUser(usuarioId, async (c) => {
      const r = await c.query(
        `SELECT l.*, cl.nome AS cliente_nome FROM lembretes l
         LEFT JOIN clientes cl ON cl.id = l.cliente_id AND cl.usuario_id = $1
         WHERE l.usuario_id = $1 ORDER BY l.criado_em DESC`, [usuarioId]);
      const out = [];
...
async function criar({ body, usuarioId, res }) {
  ...
  try {
    const resultado = await db.withUser(usuarioId, async (c) => {
      ...
      const erroVinc = await validarVinculo(c, { clienteId, emprestimoId, parcelaId }, usuarioId);
      ...
      const cobranca = await vinculoCobranca(c, rIns.rows[0], usuarioId);
...
async function atualizar({ seg, body, usuarioId, res }) {
  ...
  try {
    const resultado = await db.withUser(usuarioId, async (c) => {
      ...
      const rAtual = await c.query('SELECT cliente_id, emprestimo_id, parcela_id FROM lembretes WHERE id = $1 AND usuario_id = $2', [id, usuarioId]);
      ...
      const erroVinc = await validarVinculo(c, efetivo, usuarioId);
      if (erroVinc) return erroVinc;
      ...
      const rUpd = await c.query(
        `UPDATE lembretes SET ${setClause}, atualizado_em = now() WHERE id = $1 AND usuario_id = $${valores.length + 2} RETURNING *`, [id, ...valores, usuarioId]);
      ...
      const cobranca = await vinculoCobranca(c, rUpd.rows[0], usuarioId);
...
async function excluir({ seg, usuarioId, res }) {
  ...
  if (id === null) return falhar(res, 404, 'Lembrete não encontrado.');
  try {
    const r = await db.withUser(usuarioId, (c) => c.query(`DELETE FROM lembretes WHERE id = $1 AND usuario_id = $2 RETURNING id`, [id, usuarioId]));
...
async function listarTemplates({ usuarioId, res }) {
  try {
    const r = await db.withUser(usuarioId, (c) =>
      c.query(`SELECT chave, texto FROM lembrete_templates WHERE usuario_id = $1`, [usuarioId]));
