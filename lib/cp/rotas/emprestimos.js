...
async function listar({ query, usuarioId, res }) {
  ...
  try {
    ...
    const resultado = await db.withUser(usuarioId, async (c) => {
      ...
      const rPar = await c.query(
        `SELECT * FROM parcelas WHERE emprestimo_id = ANY($1::int[]) ORDER BY emprestimo_id, numero ASC`, [ids]);
...
async function obter({ seg, usuarioId, res }) {
  ...
  try {
    const resultado = await db.withUser(usuarioId, (c) => buscarEmprestimoCompleto(c, 'e.id = $1', [id]));
    ...
    if (e instanceof EmprestimoJaProcessado) {
      try {
        const existente = await db.withUser(usuarioId, (c) =>
          buscarEmprestimoCompleto(c, 'e.usuario_id = $1 AND e.chave_idempotencia = $2', [usuarioId, e.chave]));
...
// PUT /api/emprestimos/:id — edição protegida (loja.atualizarEmprestimo).
// Integridade financeira: com pagamentos registrados, SÓ observacoes pode
// mudar (capital/total/parcelas/vencimentos jamais — o histórico pago
// perderia o sentido). Sem pagamentos, dataOperacao também pode (o capital
// ainda não teve nenhum retorno). Empréstimo cancelado não edita.
async function atualizar({ seg, body, usuarioId, res }) {
  ...
  const proibidos = ['capital', 'total', 'parcelas', 'qtdParcelas', 'frequencia', 'clienteId', 'chaveIdempotencia']
    .filter((k) => Object.prototype.hasOwnProperty.call(corpo, k));
  if (proibidos.length) return falhar(res, 400, `O campo "${proibidos[0]}" não pode ser alterado após a criação.`);
  if (!temObs && !temData) return falhar(res, 400, 'Nenhum campo para atualizar.');
  if (temData && !dataValida(dataOperacao)) return falhar(res, 400, 'Data da operação inválida.');

  try {
    const resultado = await db.withUser(usuarioId, async (c) => {
      const rEmp = await c.query('SELECT id, status FROM emprestimos WHERE id = $1', [id]);
      if (!rEmp.rows.length) return { erro: 404, mensagem: 'Empréstimo não encontrado.' };
      if (rEmp.rows[0].status === 'cancelado') return { erro: 400, mensagem: 'Empréstimo cancelado não pode ser editado.' };

      const campos = [];
      const valores = [];
      if (temObs) { campos.push('observacoes'); valores.push(observacoes); }
      if (temData) {
        const rPag = await c.query('SELECT COUNT(*) AS n FROM pagamentos WHERE emprestimo_id = $1', [id]);
        if (Number(rPag.rows[0].n) > 0) {
          return { erro: 400, mensagem: 'Com pagamentos registrados, só as observações podem ser alteradas.' };
        }
        campos.push('data_operacao');
        valores.push(dataOperacao);
      }
      const setClause = campos.map((col, i) => `${col} = $${i + 2}`).join(', ');
      const rUpd = await c.query(
        `UPDATE emprestimos SET ${setClause}, atualizado_em = now() WHERE id = $1 RETURNING id`, [id, ...valores]);
      if (!rUpd.rows.length) return { erro: 404, mensagem: 'Empréstimo não encontrado.' };
      return buscarEmprestimoCompleto(c, 'e.id = $1', [id]);
    });
