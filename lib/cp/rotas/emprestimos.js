...
async function atualizar({ seg, body, usuarioId, res }) {
  ...
  try {
    const resultado = await db.withUser(usuarioId, async (c) => {
      const rEmp = await c.query('SELECT * FROM emprestimos WHERE id = $1', [id]);
      ...
      const rPar = await c.query(`SELECT * FROM parcelas WHERE emprestimo_id = $1 ORDER BY numero ASC`, [id]);
      ...
      if (sets.length) {
        ...
        await c.query(`UPDATE emprestimos SET ${setClause}, atualizado_em = now() WHERE id = $1`, [id, ...vals]);
      }
      if (temParcelas) {
        for (const p of novasParcelas) {
          await c.query(
            `UPDATE parcelas SET valor_centavos = $2, vencimento = $3, atualizado_em = now()
             WHERE emprestimo_id = $1 AND numero = $4`, [id, p.valor, p.vencimento, p.numero]);
        }
      }
