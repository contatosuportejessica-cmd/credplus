    ...
    const resultado = await db.withUser(usuarioId, async (c) => {
      ...
      if (meta.tipo === 'recebimento_mensal') {
        ...
        const rRec = await c.query(
          `SELECT COALESCE(SUM(valor_recebido_centavos), 0) AS recebido FROM pagamentos WHERE data BETWEEN $1 AND $2`,
          [inicio, fim]);
...
async function atualizar({ seg, body, usuarioId, res }) {
  ...
  try {
    const resultado = await db.withUser(usuarioId, async (c) => {
      ...
      const rUpd = await c.query(`UPDATE metas SET ${setClause} WHERE id = $1 RETURNING *`, [id, ...valores]);
