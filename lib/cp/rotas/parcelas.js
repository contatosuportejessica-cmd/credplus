    ...
    const resultado = await db.withUser(usuarioId, async (c) => {
      const rParcela = await c.query(
        `SELECT p.id, e.status AS emprestimo_status
         ...
         WHERE p.id = $1 AND p.usuario_id = $2 AND e.usuario_id = $2`, [id, usuarioId]);
      ...
      const r = await c.query(
        `UPDATE parcelas SET vencimento = $2, atualizado_em = now() WHERE id = $1 AND usuario_id = $3 RETURNING *`, [id, vencimento, usuarioId]);
