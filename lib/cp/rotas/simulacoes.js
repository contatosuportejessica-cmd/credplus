...
async function nomeCliente(c, clienteId) {
  ...
  try {
    ...
    const r = await db.withUser(usuarioId, (c) => c.query(
      `SELECT s.*, cl.nome AS cliente_nome FROM simulacoes s
       ...
       ${where} ORDER BY s.criado_em DESC`, params));
  ...
  try {
    const resultado = await db.withUser(usuarioId, async (c) => {
      const rSim = await c.query(
        `SELECT s.*, cl.nome AS cliente_nome FROM simulacoes s
         LEFT JOIN clientes cl ON cl.id = s.cliente_id WHERE s.id = $1`, [id]);
...
async function criar({ body, usuarioId, res }) {
  ...
  try {
    const resultado = await db.withUser(usuarioId, async (c) => {
      ...
      return { sim, clienteNome: await nomeCliente(c, dados.clienteId) };
    });
    ...
    const resultado = await db.withUser(usuarioId, async (c) => {
      ...
      const rUpd = await c.query(
        ...
        [dados.clienteId, dados.nome, dados.capital, dados.total, dados.modo, dados.percentual,
         dados.qtdParcelas, dados.frequencia, dados.diasPersonalizado, dados.dataPrimeiraCobranca, id]);
      ...
      return { sim: rUpd.rows[0], clienteNome: await nomeCliente(c, dados.clienteId) };
    });
    if (resultado.erro) return falhar(res, resultado.erro, resultado.mensagem);
    return ok(res, {
      ...montarSimulacaoResumo({ ...resultado.sim, cliente_nome: resultado.clienteNome }),
  ...
  try {
    const resultado = await db.withUser(usuarioId, async (c) => {
      ...
      const r = await c.query(
        `UPDATE simulacoes SET cliente_id = $1, atualizado_em = now() WHERE id = $2 RETURNING *`, [clienteId, id]);
      if (!r.rows.length) return { erro: 404, mensagem: 'Simulação não encontrada.' };
      return { sim: r.rows[0], clienteNome: await nomeCliente(c, clienteId) };
    });
    if (resultado.erro) return falhar(res, resultado.erro, resultado.mensagem);
...
async function arquivar({ seg, usuarioId, res }) {
  ...
  try {
    ...
    if (!r.rows.length) {
      const existe = await db.withUser(usuarioId, (c) => c.query('SELECT id FROM simulacoes WHERE id = $1', [id]));
  ...
  try {
    const resultado = await db.withUser(usuarioId, async (c) => {
      ...
      const rPar = await c.query(
        'SELECT numero, valor_centavos, vencimento FROM simulacao_parcelas WHERE simulacao_id = $1 ORDER BY numero ASC', [id]);
      ...
      const rVinculo = await c.query(
        `UPDATE simulacoes SET status = 'convertida', emprestimo_id = $1, atualizado_em = now()
         WHERE id = $2 AND status = 'em_aberto' RETURNING id`, [criado.emp.id, id]);
    ...
    if (e instanceof EmprestimoJaProcessado || e instanceof SimulacaoJaConvertida) {
      try {
        const r = await db.withUser(usuarioId, (c) => c.query('SELECT emprestimo_id FROM simulacoes WHERE id = $1', [id]));
