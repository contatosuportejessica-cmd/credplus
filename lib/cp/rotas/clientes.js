...
// Isolamento em DUAS camadas: RLS no banco (migration 003, via withUser)
// MAIS predicado explícito `usuario_id = <token>` em TODA consulta abaixo.
// O predicado explícito garante isolamento mesmo se o RLS estiver
// desabilitado no banco — nunca confia só numa camada. usuarioId vem
// sempre do JWT verificado, nunca do corpo da requisição.
const db = require('../db');
...
async function obter({ seg, usuarioId, res }) {
  ...
  try {
    const r = await db.withUser(usuarioId, (c) =>
      c.query(`SELECT ${COLUNAS_RETORNO.join(', ')} FROM clientes WHERE id = $1 AND usuario_id = $2`, [id, usuarioId]));
...
async function atualizar({ seg, body, usuarioId, res }) {
  ...
  try {
    const sql = `UPDATE clientes SET ${setClause}, atualizado_em = now() WHERE id = $1 AND usuario_id = $${valores.length + 2} RETURNING ${COLUNAS_RETORNO.join(', ')}`;
    const r = await db.withUser(usuarioId, (c) => c.query(sql, [id, ...valores, usuarioId]));
    if (!r.rows.length) return falhar(res, 404, 'Cliente não encontrado.');
...
async function arquivar({ seg, usuarioId, res }) {
  ...
  try {
    const r = await db.withUser(usuarioId, (c) =>
      c.query(`UPDATE clientes SET status = 'arquivado', arquivado_em = now(), atualizado_em = now() WHERE id = $1 AND usuario_id = $2 RETURNING id`, [id, usuarioId]));
    if (!r.rows.length) return falhar(res, 404, 'Cliente não encontrado.');
