...
async function anexarItens(c, pagamentos, usuarioId) {
  ...
  const r = await c.query(
    `SELECT i.pagamento_id, i.parcela_id, i.valor_centavos, p.numero
     ...
     WHERE i.pagamento_id = ANY($1::int[]) AND i.usuario_id = $2 ORDER BY p.numero ASC`, [ids, usuarioId]);
  const mapa = new Map();
...
async function listar({ query, usuarioId, res }) {
  ...
  try {
    ...
    const resultado = await db.withUser(usuarioId, async (c) => {
      ...
      const mapa = await anexarItens(c, r.rows, usuarioId);
...
async function registrar({ body, usuarioId, res }) {
  ...
  try {
    const resultado = await db.withUser(usuarioId, async (c) => {
      ...
      const rPar = await c.query(
        `SELECT p.id, p.numero, p.status AS parcela_status, p.valor_centavos, p.valor_pago_centavos,
         ...
         WHERE p.id = ANY($1::int[]) AND p.usuario_id = $2 AND e.usuario_id = $2 FOR UPDATE OF p`, [ids, usuarioId]);
      ...
      for (const it of itensEntrada) {
        ...
        await c.query(`UPDATE parcelas SET valor_pago_centavos = $2, atualizado_em = now() WHERE id = $1 AND usuario_id = $3`, [it.parcelaId, novo, usuarioId]);
      }
    ...
    const mapa = await db.withUser(usuarioId, (c) => anexarItens(c, [resultado.pagamento], usuarioId));
