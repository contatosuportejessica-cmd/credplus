...
async function clienteExiste(c, clienteId, usuarioId) {
...
async function listar({ seg, usuarioId, res }) {
  ...
  try {
    const resultado = await db.withUser(usuarioId, async (c) => {
      if (!(await clienteExiste(c, clienteId, usuarioId))) return null;
      const r = await c.query(
        `SELECT id, cliente_id, nome_original, mime, tamanho_bytes, criado_em
         FROM arquivos WHERE cliente_id = $1 AND usuario_id = $2 ORDER BY criado_em DESC`, [clienteId, usuarioId]);
...
async function enviar({ seg, body, usuarioId, res }) {
  ...
  try {
    const resultado = await db.withUser(usuarioId, async (c) => {
      if (!(await clienteExiste(c, clienteId, usuarioId))) return { erro: 404, mensagem: 'Cliente não encontrado.' };
      const r = await c.query(
...
async function baixar({ seg, usuarioId, res }) {
  ...
  try {
    const resultado = await db.withUser(usuarioId, (c) =>
      c.query(`SELECT nome_original, mime, dados FROM arquivos WHERE id = $1 AND cliente_id = $2 AND usuario_id = $3`, [arquivoId, clienteId, usuarioId]));
...
async function excluir({ seg, usuarioId, res }) {
  ...
  try {
    const r = await db.withUser(usuarioId, (c) =>
      c.query(`DELETE FROM arquivos WHERE id = $1 AND cliente_id = $2 AND usuario_id = $3 RETURNING id`, [arquivoId, clienteId, usuarioId]));
