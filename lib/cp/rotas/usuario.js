...
async function backup({ usuarioId, res }) {
  try {
    const dados = await db.withUser(usuarioId, async (c) => {
      ...
      const clientes = (await c.query('SELECT * FROM clientes WHERE usuario_id = $1 ORDER BY criado_em DESC', [usuarioId])).rows;
      const emprestimos = (await c.query('SELECT * FROM emprestimos WHERE usuario_id = $1 ORDER BY criado_em DESC', [usuarioId])).rows;
      const parcelas = (await c.query('SELECT * FROM parcelas WHERE usuario_id = $1 ORDER BY emprestimo_id, numero ASC', [usuarioId])).rows;
      const pagamentos = (await c.query('SELECT * FROM pagamentos WHERE usuario_id = $1 ORDER BY criado_em DESC', [usuarioId])).rows;
      const movimentacoes = (await c.query('SELECT * FROM movimentacoes_financeiras WHERE usuario_id = $1 ORDER BY data DESC, criado_em DESC', [usuarioId])).rows;
      const lembretes = (await c.query('SELECT * FROM lembretes WHERE usuario_id = $1 ORDER BY criado_em DESC', [usuarioId])).rows;
      const metas = (await c.query('SELECT * FROM metas WHERE usuario_id = $1 ORDER BY criado_em DESC', [usuarioId])).rows;
      const notas = (await c.query('SELECT * FROM notas WHERE usuario_id = $1 ORDER BY criado_em DESC', [usuarioId])).rows;
      const simulacoes = (await c.query('SELECT * FROM simulacoes WHERE usuario_id = $1 ORDER BY criado_em DESC', [usuarioId])).rows;
      const simulacaoParcelas = (await c.query(
        'SELECT * FROM simulacao_parcelas WHERE usuario_id = $1 ORDER BY simulacao_id, numero ASC', [usuarioId])).rows;
      return {
