  ...
  try {
    const resultado = await db.withUser(usuarioId, async (c) => {
      ...
      const rUpd = await c.query(
        `UPDATE notas SET ${setClause}, atualizado_em = now() WHERE id = $1 RETURNING *`, [id, ...valores]);
...
async function excluir({ seg, usuarioId, res }) {
  ...
  if (id === null) return falhar(res, 404, 'Nota não encontrada.');
  try {
    const r = await db.withUser(usuarioId, (c) => c.query(`DELETE FROM notas WHERE id = $1 RETURNING id`, [id]));
