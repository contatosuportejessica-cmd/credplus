...
function getPool() {
  if (!pool) {
    pool = new Pool({
      ...
      // antes do próprio Pool fechá-la sozinho. 5s (valor do exemplo
      // genérico da doc) media menos que o tempo real entre um clique e
      // outro de uma pessoa navegando — a conexão fechava sozinha entre
      // cliques e todo clique seguinte pagava reconexão de novo (medido ao
      // vivo: ~850ms em vez de ~115ms). 60s cobre o intervalo normal de
      // navegação sem segurar conexão por muito tempo — e mesmo se a
      // instância for suspensa antes disso, `attachDatabasePool` (abaixo)
      // já garante que a conexão é fechada direito, não vazada.
